...
namespace SteelEngine {
	...
	void Serializer::SetISerializedValue(const char* propertyName, const HotReloader::ISerializedValue* pValue)
	{
	...
	const HotReloader::ISerializedValue* Serializer::GetISerializedValue(const char* propertyName) const
	{
		assert(m_CurrentObject);
		assert(propertyName);
		assert(m_IsLoading);

		const HotReloader::ISerializedValue* pRet = NULL;
	...
	void Serializer::Serialize(HotReloader::IRuntimeObject* object)
	{
		assert(object);
