...
namespace SteelEngine { namespace Editor { namespace ImGUI {
    ...
    ImGUI_Editor::ImGUI_Editor()
    {
        ...
        for(Type::uint32 i = 0; i < size; i++)
        {
            ...
            Variant* apiVariant = type->GetMetaData(API::CONTEXT_API);

            if(apiVariant && apiVariant->IsValid())
            {
                if(Utils::RenderContext::GetCurrentAPI() == apiVariant->Convert<Graphics::IRenderer::API>())
                {
                    m_API_Context = (IContext*)type->Create();
                    m_ContextType = type;
    ...
    Result ImGUI_Editor::Init(Utils::RenderContext* renderContext, IWindow* window)
    {
        if(m_API_Context)
        {
            m_API_Context->Init(m_ContextType, window, renderContext->GetRenderDevice());

            SE_INFO("Editor API context initialization success!");

            m_ContextType->Invoke("MakeCurrent", m_API_Context);
            m_Context = (ImGuiContext*)m_ContextType->Invoke("GetContext", m_API_Context).Convert<void*>();
        }
        else
        {
    ...
    // Here should be another context but only for imgui - internal
        ...
        for(HotReloader::InheritanceTrackKeeper* swapper : m_Windows)
        {
            EditorComponents::ImGUI::UserInterface* window = swapper->Get<EditorComponents::ImGUI::UserInterface>();
        ...
        IEditor::Init(*m_VirtualProjectVisualizer, m_API_Context);
    ...
    void ImGUI_Editor::Draw(void* finalTexture)
    {
        m_API_Context->Update();
        ...
        if(m_IsViewportOpen)
        {
            ...
            else
            {
                ...
                ImGui::Image(finalTexture, m_CurrentMainViewportSize, { 0, 1 }, { 1, 0 });
    ...
    {
        event.m_Window->Get<EditorComponents::ImGUI::UserInterface>()->m_Context = m_Context;
